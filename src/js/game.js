...
const createPlaceholdersHTML = () => {
    ...
    for (let i = 0; i < word.length; i++) {
        placeholderHTML += `<h1 id="letter_${i}" class="letter">_</h1>`;
    }
    ...
    if (!word.includes(inputLetter)) {
        ...
        wordArray.forEach((currentLetter, i) => {
            if (currentLetter === inputLetter) {
                ...
                document.getElementById(`letter_${i}`).innerText = currentLetter.toUpperCase();
            }
        })
    }
}
...
export const startGame = () => {
    ...
    const wordToGeess = WORDS[randomIndex];
    sessionStorage.setItem('word', wordToGeess);

    gameDiv.innerHTML = createPlaceholdersHTML();

    gameDiv.innerHTML += `<p id="tries" class="mt-2">TRIES LEFT: <span id="tries-left" class="font-medium text-red-600 text-sl">10</span></p>`;
